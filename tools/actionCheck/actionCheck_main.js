  ...
  checkPass(uid, clearTime = 5 * 60 * 1000, deltaTime = 3000) {
    ...
    if (!user.timer) {
      user.timer = setTimeout(function() {
        user.come = 0;
        return console.log(`用户uid：${uid}的频繁统计已清除`);
      }, clearTime);
    }
    user.come += 1;
          ...
          (async() => {            //这里无需等待，直接继续
            ...
            try {
              return (await mail.send({
                to: email,
                subject: `${siteBaseName}验证码`,
                html: `<h1>你的${siteBaseName}授权操作的验证码为：</h1>
<p>${code}<p>`
              }));
            } catch (error) {
              err = error;
            ...
            (async() => {              //这里无需等待，直接继续
              ...
              try {
                return (await mail.send({
                  to: email,
                  subject: `${siteBaseName}验证码`,
                  html: `<h1>你的${siteBaseName}验证码为：</h1>
<p>${code}<p>`
                }));
              } catch (error) {
                err = error;
